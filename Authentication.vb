...
Public Class Authentication
    ...
    Public Shared Function IsPasswordValid(encryptedPassword As Byte(), password As String) As Boolean
        If (_byteLength < 1) Then
            Throw New Exception($"{NameOf(_byteLength)} cannot be less than 1")
        End If
        If (Not encryptedPassword.Any()) Then
            Throw New ArgumentNullException(NameOf(encryptedPassword))
        End If
        If (encryptedPassword.Length < _byteLength) Then
            Throw New ArgumentOutOfRangeException(NameOf(encryptedPassword))
        End If
        If (String.IsNullOrEmpty(password)) Then
            Throw New ArgumentNullException(NameOf(password))
        End If

        Dim salt = encryptedPassword.Take(_byteLength).ToArray()
        Dim hash = GenerateHash(password, salt)
        Dim combinedHash = CombineSaltAndHash(salt, hash)

        Dim counter = 0
        Dim passwordsMatch = True
        Do While counter < encryptedPassword.Length AndAlso counter < combinedHash.Length
            If (passwordsMatch) Then
                passwordsMatch = encryptedPassword(counter).Equals(combinedHash(counter))
            Else
                ' this is not necessary, but added as a counter measure to timing attacks
                encryptedPassword(counter).Equals(combinedHash(counter))
            End If
